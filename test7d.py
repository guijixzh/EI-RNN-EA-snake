# ==========================================
# test7d.py —— 适应度级动作效率筛选（双臂对照版，基于 test7c）
#
# 相对 test7c 的改动：
#  1. 单侧转弯判死默认关闭（ONE_SIDED_TURN_DEATH=False）：
#     早期随机个体普遍摇头，该判罚干扰初期筛选；CLI --one-sided-death 可重开。
#  2. 指标新增 turns_last（第 10 列）：吃下最后一颗食物为止的累计转向数。
#  3. FIT_MODE 双臂：
#     econ     fitness = food * min(STEP_REF*food / (steps_last + TURN_COST*turns_last), 1)
#              乘法经济学：动作成本 2 倍于最优时适应度砍半，全分段强梯度。
#     additive fitness = food + FOOD_EFF_WEIGHT*food/steps_last - TURN_PENALTY*turn_ratio
#              加法惩罚：转向占比线性扣分（tie-breaker 量级）。
#  4. 实验规格：随机初始化（SEED_FROM_BEST=False）、20 代；
#     文件前缀按臂自动隔离 test7d_econ_* / test7d_add_*。
# 转向疲劳（gain=0.2/decay=0.9）、饿死斜率（3*len+20）维持 test7c 值。
# ==========================================
...
class Config:
    ...
    # --- 断点 / 最优模型 / 种子 ---
    CHECKPOINT_PATH = 'test7d_econ_checkpoint.pth'
    BEST_MODEL_PATH = 'test7d_econ_best_model.pth'
    LATEST_GEN_BEST_MODEL_PATH = 'test7d_econ_latest_gen_best.pth'
    AUTO_RESUME = True
    CHECKPOINT_INTERVAL = 10
    SEED_FROM_BEST = False
    SEED_MODEL_PATH = 'test7c_latest_gen_best.pth'
    SEED_MODEL_PATH2 = 'test7c_best_model.pth'
...
def _fitness_econ(m, cfg):
    """乘法经济学适应度：
    cost = steps_last + TURN_COST*turns_last
    fitness = food * min(STEP_REF*food/max(cost,1), 1)
    动作成本 2 倍于最优时适应度砍半；上限 1 保证不奖励慢性子。
    m = 指标行 [10]：0 food, 1 seen, 2 unseen, 3 steps_last, 4 prox, 8 act1, 9 act2
    """
    if m[1] >= 99999:
        return -1e9
    food, steps_last, turns_last = m[0], m[3], m[10]
    cost = steps_last + float(getattr(cfg, 'TURN_COST', 1.0)) * turns_last
...
def make_smoke_config():
    ...
    cfg.CHECKPOINT_PATH = 'test7d_smoke_checkpoint.pth'
    cfg.BEST_MODEL_PATH = 'test7d_smoke_best.pth'
    cfg.LATEST_GEN_BEST_MODEL_PATH = 'test7d_smoke_latest_gen_best.pth'
...
def main():
    ap = argparse.ArgumentParser(description='test7d — 适应度级动作效率筛选双臂对照（基于 test7c）')
    ...
    ap.add_argument('--fit-mode', type=str, default=None, choices=['econ', 'additive', 'tuple'],
                    help='适应度模式（默认 econ）；决定文件前缀 test7d_econ_*/test7d_add_*')
    ...
    if args.fit_mode:
        ...
        if not args.smoke:
            ...
            cfg.CHECKPOINT_PATH = f'test7d_{arm}_checkpoint.pth'
            cfg.BEST_MODEL_PATH = f'test7d_{arm}_best_model.pth'
            cfg.LATEST_GEN_BEST_MODEL_PATH = f'test7d_{arm}_latest_gen_best.pth'
