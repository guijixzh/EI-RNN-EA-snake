# ==========================================
# test16c.py —— W_in/W_out 尺寸约束（传感池/运动池）制造"隐藏层"；test16b 子版本
#
# 假设（win_wout 分布分析，results/win_wout_distribution.png）：test7b/16b 训练后
#   99%+ 的柱都直连观测、反射弧占 37-40%、纯内部柱仅 1-2%——"隐藏层"从未被
#   选择造出来，因为观测连接免费且初始化全柱皆连。本轮用结构性约束制造生态位：
#
# 传感池/运动池（固定解剖结构，全体个体共用，seed=20260907 确定性生成）：
#   SENSORY_FRAC=0.35 —— 仅 35% 的柱允许拥有 M_in（读观测）；
#   MOTOR_FRAC=0.25   —— 仅 25% 的柱允许拥有 M_out（驱动动作）；
#   独立采样 → 反射弧≈8.75%、纯内部柱≈46%（对照 test16b：37% / 1%）。
#   random_init 应用池掩码；变异期 M_in/M_out 翻转仅在池内生效（池边界守卫）；
#   rec 槽位（K=16）与 W_rec 不受限——信息必须走"传感柱→内部柱→运动柱"多跳链路。
#
# 其余与 test16b 逐字一致（pop4096/精英1024=1/4、K=16、K1=12+对半精评(6/18)
#   精英 K=36、simple 适应度、LCB、fast-eval）。对照基线 = test16b_simp 前 20 代。
# 验证假设：同预算同代数下，池约束臂的 Avg/Elite/Best 轨迹是否不降反升
#   （深度化收益 > 感知带宽损失）；20 代后如 ≈ 基线则约束无害，可放长跑。
# ==========================================
...
class Config:
    ...
    # --- 脑结构参数（test16：稀疏固定扇入）---
    NUM_COLUMNS = 1024
    REC_FANIN = 64              # 每突触后神经元输入槽数 K（≤ N-1）
    EXPAND_K_OLD = 16           # rec 遥测新槽分界（同 test16b §9）：K>此值时
                                # newSlotW/newSlotWPop 统计槽位 ≥ 此值的质量占比；
                                # 原生 K=64 run 无扩容语义，仅作分区基线（约 75%）
    OBS_MODE = '32proj'
    OBS_DIM = 24 if OBS_MODE == '24' else 40    # test15 血统：32 旧通道 + 8 新通道
    ACTION_DIM = 3
    INIT_DENSITY = 0.15         # 仅用于 M_in/M_out（rec 由 REC_FANIN 决定）
    # --- test16c：W_in/W_out 尺寸约束（传感池/运动池）---
    SENSORY_FRAC = 0.5         # 允许读观测的柱占比（0=关，回退 test16b）
    MOTOR_FRAC = 0.25           # 允许驱动动作的柱占比（0=关）
    ...
    # --- 断点 / 最优模型 / 种子 ---
    CHECKPOINT_PATH = 'test16c_checkpoint.pth'
    BEST_MODEL_PATH = 'test16c_best_model.pth'
    LATEST_GEN_BEST_MODEL_PATH = 'test16c_latest_gen_best.pth'
    HISTORY_JSON_PATH = 'test16c_history.json'
    AUTO_RESUME = True
...
def _pool_masks(cfg, N, device):
    ...
    SENSORY_FRAC/MOTOR_FRAC 任一为 0 时返回全 True（=无约束，回退 test16b）。"""
...
class GeneStack:
    ...
    def random_init(self):
        ...
        with torch.no_grad():
            ...
            smask, mmask = _pool_masks(cfg, N, dev)      # test16c 池约束
...
def rec_telemetry(rec_idx_row, rec_w_row, k_old=16):
    ...
    新槽权占比 = 槽位 ≥ k_old 的 |rec_w| 质量占总质量比（同 test16b §9 遥测；
    K≤k_old 时返回 -1）。"""
    with torch.no_grad():
...
def pop_param_telemetry(pop, k_old=16):
    ...
    返回：新槽权质量占比的 pop 均值/最大（nP/nPx）、新槽非零率（nNz）、
    G2 基因（tau_e/w_ei/w_ie）的种群均值±标准差。K≤k_old 时新槽三项返回 -1。"""
    with torch.no_grad():
        ...
        if has_g1:
            ...
            for ai, attr in enumerate(GeneStack.G1_MASKS):
                ...
                if attr == 'rec_idx':
                    ...
                    if allow is not None:      # test16c 池边界守卫：池外翻转无效
                        flip = flip & allow
                    setattr(children, attr,
    ...
    try:
        ...
        req = urllib.request.Request(
            'https://www.autodl.com/api/v1/wechat/message/send',
                data=json.dumps({'title': title[:20], 'name': 'test16c 提速评估进化',
...
def run_training(cfg):
    ...
    print(f"[test16c 池] 传感池 {g_in:.0%} / 运动池 {g_out:.0%}"
          f"（反射弧≈{g_in*g_out:.0%}，纯内部≈{1-g_in-g_out+g_in*g_out:.0%}，"
          f"seed={getattr(cfg,'POOL_SEED',0)}）")
    ...
    try:
        for gen in range(start_gen, cfg.GENERATIONS):
            ...
            # best 个体循环连接遥测 + 种群层面遥测（同 test16b §9）
...
def selfcheck(cfg):
    ...
    print("=== 自检 9：传感/运动池完整性（test16c）===")
    ok9 = True
    sc9 = Config()
    sc9.NUM_COLUMNS, sc9.SENSORY_FRAC, sc9.MOTOR_FRAC = 64, 0.35, 0.25
    sc9.POP_SIZE, sc9.ELITE_SIZE, sc9.STAGE2_KEEP = 16, 4, 8
    sc9.OBS_DIM, sc9.ACTION_DIM = 40, 3
    sc9.CYCLE_PATTERN = [('G2', 'G1')]
    sc9.TOPOLOGY_MUT_PROB, sc9.MUT_RATE = 1.0, 1.0
    ...
    ok9 = ok9 and bad_in == 0 and bad_out == 0
    fake_m = torch.zeros(16, 19)
    p9n = evolve_topology_gpu(p9, fake_m, sc9, gen=1)
    nm_in = p9n.M_in[4:]
    nm_out = p9n.M_out[4:]
    bad_in2 = int((nm_in[:, ~sin9, :] != 0).sum())
    bad_out2 = int((nm_out[:, :, ~mot9] != 0).sum())
    print(f"  全强度变异后子代(12个): 池外 M_in {bad_in2} / M_out {bad_out2}（期望 0/0）")
    ok9 = ok9 and bad_in2 == 0 and bad_out2 == 0
    frac = float(sin9.float().mean())
    tol = 2.0 * (0.35 * 0.65 / 64) ** 0.5     # 2σ 伯努利采样容差（N=64 小样本）
    print(f"  传感池实际占比 {frac:.2f}（目标 0.35±{tol:.2f}）"
          f"{'OK' if abs(frac-0.35)<tol else 'FAIL'}")
    print(f"  池完整性 {'OK' if ok9 else 'FAIL'}")
    print("=== 自检完成 ===")
...
def make_smoke_config():
    ...
    cfg.CHECKPOINT_PATH = 'test16c_smoke_checkpoint.pth'
    cfg.BEST_MODEL_PATH = 'test16c_smoke_best.pth'
    cfg.LATEST_GEN_BEST_MODEL_PATH = 'test16c_smoke_latest_gen_best.pth'
    cfg.HISTORY_JSON_PATH = 'test16c_smoke_history.json'
...
def main():
    ...
    ap.add_argument('--sensory-frac', type=float, default=None,
                    help='test16c：允许读观测的柱占比（默认 0.35；0=关池约束）')
    ap.add_argument('--motor-frac', type=float, default=None,
                    help='test16c：允许驱动动作的柱占比（默认 0.25；0=关池约束）')
    ...
    if args.fit_mode:
        ...
        if not args.smoke:
            ...
            cfg.CHECKPOINT_PATH = f'test16c_{arm}_checkpoint.pth'
            cfg.BEST_MODEL_PATH = f'test16c_{arm}_best_model.pth'
            cfg.LATEST_GEN_BEST_MODEL_PATH = f'test16c_{arm}_latest_gen_best.pth'
            cfg.HISTORY_JSON_PATH = f'test16c_{arm}_history.json'
    ...
    if args.name:
        cfg.CHECKPOINT_PATH = f'test16c_{args.name}_checkpoint.pth'
        cfg.BEST_MODEL_PATH = f'test16c_{args.name}_best_model.pth'
        cfg.LATEST_GEN_BEST_MODEL_PATH = f'test16c_{args.name}_latest_gen_best.pth'
        cfg.HISTORY_JSON_PATH = f'test16c_{args.name}_history.json'
    if args.resume_pop:
    ...
    try:
        ...
        send_autodl_notify(cfg, 'test16c 训练异常退出',
