# ==========================================
# test14.py —— 期相激素 v1：小网络的局内动态调制（test12 血统 + G3 激素组）
#
# 动机（Phase-0 相位归因，results/phase_diagnostic.json）：冠军脑从后期身体
# 直接出生续评即崩塌（7b teacher L=30 ratio=0.00；test12 L=50 ratio≈0.3，
# 主死因空间挤压自撞）——"前后期质变、静态权重压不下两种模式"获门确认。
#
# 机制 v1（保留 test4a 全网扩散骨架，修死通道三处）：
#   - 释放：逐柱连续量 rel = tanh(W_rel·期相特征 + b_rel)，无 argmax/阈值门控；
#     W_rel/b_rel 非零小尺度初始化（σ=HORMONE_INIT_STD），变异 gen 0 全覆盖
#     （test5d 死通道 = 0 初始化 + 阈值门 + 200 代后解冻，选择清洗一切变异）。
#   - 读取：Fin=6 期相特征（len/G²、饥饿钟压力、头尾距、mean E/I、steps）
#     线性逐柱直读，无隐层（~3k 新基因/脑；conn/reach 需 G² 池化迭代，不逐步喂）。
#   - 场动力学：horm = (1-λ)·rel + λ·((1-D)·horm + D·(M_rec·horm)/deg)，
#     每环境步一次（每局清零）；λ=HORMONE_DECAY，D=EXCIT/INHIB_DIFFUSION。
#   - 调制目标：tau_e（兴奋↑/抑制↓，语义=柱时间尺度=反应⇄规划换挡）+
#     输入群组增益 ext×(1+κ·(horm_e−horm_i))（感觉重加权）。
# 消融开关：HORMONE_ENABLE（总闸，评估期关=与 test12 严格同数学），
#   HORMONE_GAIN_ENABLE（增益目标分闸）。
# mini A/B 预注册（experiments/exp_hormone_ab.py）：C0 零激素对照 / H1 冻结底盘
#   只进化激素基因 / H2 全协同；判定：elite_food 相对 C0 ≥ +1.5 且消融载重
#   （评估期关激素掉分 ≥1）→ 转长跑。BRAIN_VERSION='horm1' 断点守卫。
# ==========================================
...
class Config:
    ...
    # --- 期相激素 v1（G3 组，test4a 全网扩散骨架的批量化重写）---
    BRAIN_VERSION = 'horm1'     # 基因组版本守卫（与 test12 断点互斥）
    HORMONE_ENABLE = True       # 总闸：False = 与 test12 严格同数学（消融用）
    HORMONE_FIN = 6             # 期相特征数（见 hormone_features）
    HORMONE_INIT_STD = 0.02     # 激素基因非零初始化（避开 test5d 死通道）
    HORMONE_DECAY = 0.95        # 场衰减 λ（每环境步）
    EXCIT_DIFFUSION = 0.4       # 兴奋激素沿拓扑扩散系数（test4a 标定）
    INHIB_DIFFUSION = 0.1       # 抑制激素扩散系数（更局部）
    HORMONE_TAU_E_GAIN = 0.25   # 兴奋激素 ↑tau（记忆巩固方向）
    HORMONE_TAU_I_GAIN = 0.50   # 抑制激素 ↓tau（反应方向）
    HORMONE_GAIN_ENABLE = True  # 输入群组增益分闸
    HORMONE_GAIN_SCALE = 0.5    # ext×(1+κ·(horm_e−horm_i)) 的 κ
    HORMONE_APPLY_EVERY = 1     # 每 n 个环境步更新一次激素场（成本旋钮）
    HORMONE_MUT_STD = 0.05      # 激素基因高斯变异 std
    HORMONE_MUT_FRAC = 0.3      # 激素基因变异覆盖比例
    FREEZE_BASE = False         # H1 臂：底盘基因（G1/G2）完全冻结

    # --- 断点 / 最优模型 / 种子 ---
    CHECKPOINT_PATH = 'test14_econ_checkpoint.pth'
    BEST_MODEL_PATH = 'test14_econ_best_model.pth'
    LATEST_GEN_BEST_MODEL_PATH = 'test14_econ_latest_gen_best.pth'
    HISTORY_JSON_PATH = 'test14_econ_history.json'
    AUTO_RESUME = True
...
class GeneStack:
    ...
    def random_init(self):
        ...
        with torch.no_grad():
            ...
            # 期相激素 v1：非零小尺度初始化（死通道对策——变异从 gen 0 即有分级行为效应）
            hs = float(getattr(cfg, 'HORMONE_INIT_STD', 0.02))
            F = int(getattr(cfg, 'HORMONE_FIN', 6))
            self.W_rel_e = torch.randn(B, N, F, device=dev) * hs
            self.W_rel_i = torch.randn(B, N, F, device=dev) * hs
            self.b_rel_e = torch.randn(B, N, device=dev) * hs
            self.b_rel_i = torch.randn(B, N, device=dev) * hs
        self.dtype = torch.float32
    ...
    def individual_state(self, i, use_half=False):
        ...
        with torch.no_grad():
            st = {
                ...
                'W_rel_e': self.W_rel_e[i].to(cpu, dtype=dt),
                'W_rel_i': self.W_rel_i[i].to(cpu, dtype=dt),
                'b_rel_e': self.b_rel_e[i].to(cpu, dtype=dt),
                'b_rel_i': self.b_rel_i[i].to(cpu, dtype=dt),
            }
        return st
    ...
    def set_individual_from_state(self, i, st):
        ...
        with torch.no_grad():
            ...
            # 血统迁移：test12 旧格式无激素键 → 保持 random_init 的非零小尺度
            if 'W_rel_e' in st:
                self.W_rel_e[i] = st['W_rel_e'].float().to(dev)
                self.W_rel_i[i] = st['W_rel_i'].float().to(dev)
                self.b_rel_e[i] = st['b_rel_e'].float().to(dev)
                self.b_rel_i[i] = st['b_rel_i'].float().to(dev)
...
# ==========================================
# 3. 批量前向（E-I 动力学 + 期相激素 v1）
# ==========================================
def hormone_features(env, E, I, cfg):
    """期相特征 [B,F]（全部 O(1) 张量运算；conn/reach 需 G² 池化迭代，不逐步喂）：
    0 蛇长/G²；1 饥饿钟压力 steps_wo_food/(SLOPE·len+20)；2 头尾距/(2G)；
    3 mean E；4 mean I；5 steps/2000。全部 clamp 到 [0,1]。"""
    B, G, dev = env.B, env.G, env.device
    ar = torch.arange(B, device=dev)
    head = env.head.float()
    tail = env.body[ar, (env.body_len - 1).clamp(min=0)].float()
    ht = (head - tail).abs().sum(dim=1) / (2.0 * G)
    clock = (env.steps_wo_food.float()
             / (float(cfg.STARVE_SLOPE) * env.body_len.float() + 20.0))
    feats = torch.stack((
        env.body_len.float() / float(G * G),
        clock.clamp(0.0, 1.0),
        ht.clamp(0.0, 1.0),
        E.mean(dim=1).float(),
        I.mean(dim=1).float(),
        (env.steps.float() / 2000.0).clamp(0.0, 1.0),
    ), dim=1)
    return feats.to(E.dtype)


def hormone_step(pop, feats, horm, cfg):
    """激素场更新（每环境步一次）：释放 + 沿 M_rec 拓扑扩散衰减。

    释放 = tanh(W_rel·feats + b_rel)：逐柱连续量，无 argmax/阈值门控
    （test5d 死通道对策之二）；非零小尺度初始化 ⇒ 初期场≈0，零激素基线可复现，
    但每个变异都有分级行为效应供选择。horm={'e','i'} [B,N]，返回新场。"""
    rel_e = torch.tanh(torch.bmm(pop.W_rel_e, feats.unsqueeze(-1)).squeeze(-1)
                       + pop.b_rel_e)
    rel_i = torch.tanh(torch.bmm(pop.W_rel_i, feats.unsqueeze(-1)).squeeze(-1)
                       + pop.b_rel_i)
    lam = float(cfg.HORMONE_DECAY)
    deg = pop.M_rec.sum(dim=2).clamp(min=1.0)   # [B,N] 行入度（拓扑扩散归一化）
    out = {}
    for k, rel, dif in (('e', rel_e, float(cfg.EXCIT_DIFFUSION)),
                        ('i', rel_i, float(cfg.INHIB_DIFFUSION))):
        h = horm[k]
        spread = torch.bmm(pop.M_rec, h.unsqueeze(-1)).squeeze(-1) / deg
        out[k] = (1.0 - lam) * rel + lam * ((1.0 - dif) * h + dif * spread)
    return out


def forward_batch(pop, obs, E, I, st, press, cfg, horm=None):
    """单次 E-I 迭代（B 个个体并行）。horm=None 或 HORMONE_ENABLE=False 时
    与 test12 严格同数学；启用时调制 tau_e（兴奋↑/抑制↓）与输入群组增益
    ext×(1+κ·(horm_e−horm_i))。FATIGUE_TURN_GAIN=0 时 press 惩罚项为 0。"""
    he = horm is not None and bool(getattr(cfg, 'HORMONE_ENABLE', False))
    ext = torch.bmm(pop.W_in_eff, obs.unsqueeze(-1)).squeeze(-1)
    rec = torch.bmm(pop.W_rec_eff, E.unsqueeze(-1)).squeeze(-1)
    if he and bool(getattr(cfg, 'HORMONE_GAIN_ENABLE', False)):
        ext = ext * (1.0 + float(cfg.HORMONE_GAIN_SCALE) * (horm['e'] - horm['i']))
    total = ext + rec

    st = cfg.SHORT_TERM_DECAY * st + (1 - cfg.SHORT_TERM_DECAY) * E
    if he:
        tau = (pop.tau_e + cfg.SHORT_TERM_GAIN * st
               + float(cfg.HORMONE_TAU_E_GAIN) * horm['e']
               - float(cfg.HORMONE_TAU_I_GAIN) * horm['i']
...
def deliberate_batch(pop, obs, E, I, st, press, cfg, horm=None):
    """K 倍帧率思考：内部迭代 K 次，logits 平均后 argmax。"""
    K = cfg.FRAME_RATE
    logits_sum = None
    for k in range(K):
        o = obs * (cfg.INPUT_DECAY ** k)
        logits, E, I, st = forward_batch(pop, o, E, I, st, press, cfg, horm)
...
def _eval_sweep_chunk(pop_rep, cfg, bank):
    ...
    bank 中 e_id 对应库的局。返回 [pop_rep.P, 21] 单局指标（列19/20=激素场
    强度遥测，HORMONE_ENABLE=False 时恒 0）。"""
    B = pop_rep.P
    dev = pop_rep.device
    N = pop_rep.N
    ...
    # 期相激素 v1：场状态（每局清零）+ 遥测累计器
    horm = None
    if bool(getattr(cfg, 'HORMONE_ENABLE', False)):
        horm = {'e': torch.zeros(B, N, dtype=half, device=dev),
                'i': torch.zeros(B, N, dtype=half, device=dev)}
    horm_every = max(1, int(getattr(cfg, 'HORMONE_APPLY_EVERY', 1)))
    tot_he = torch.zeros(B, dtype=torch.float32, device=dev)
    tot_hi = torch.zeros(B, dtype=torch.float32, device=dev)
    ...
    for t in range(cfg.MAX_STEPS):
        ...
        if horm is not None and t % horm_every == 0:
            feats = hormone_features(env, E, I, cfg)
            horm = hormone_step(pop_rep, feats, horm, cfg)
        act, E, I, st = deliberate_batch(pop_rep, obs, E, I, st, press, cfg, horm)
        if horm is not None:
            tot_he += al.float() * horm['e'].abs().mean(dim=1).float()
            tot_hi += al.float() * horm['i'].abs().mean(dim=1).float()
    ...
    horm_e_norm = tot_he / beh_steps.clamp(min=1.0)
    horm_i_norm = tot_hi / beh_steps.clamp(min=1.0)
...
def _eval_pop_banks(pop, cfg, banks):
    ...
    返回 [P,21] = E 局均值（12=mismatch，13=min_reach，14=island（诊断）；
    ...
    per_P = max(1, max_B // E)
    ncol = 21    # 19=edge_share 后追加 19=horm_e_norm 20=horm_i_norm（激素遥测）
...
def evaluate_population_gpu(pop, cfg, gen=0):
    ...
    返回 (metrics[P,21], order[P])：order = 幸存者按累计适应度降序，
...
def evolve_topology_gpu(pop, metrics, cfg, gen=0, order=None):
    ...
    has_g1 = 'G1' in active
    has_g2 = 'G2' in active
    freeze_base = bool(getattr(cfg, 'FREEZE_BASE', False))   # H1 臂：底盘冻结
    ...
    horm_frac1 = (cfg.HORMONE_MUT_FRAC * s).clamp(max=1.0)                # [B2]
    horm_std1 = (cfg.HORMONE_MUT_STD * s)                                 # [B2]
    ...
    with torch.no_grad():
        ...
        if has_g1:
            if freeze_base:
                # H1：底盘基因整体取 p1（无交叉无变异，仅 G3 激素进化）
                for attr in GeneStack.G1_WEIGHTS + GeneStack.G1_MASKS:
                    setattr(children, attr, getattr(p1, attr).clone())
            else:
        ...
        if has_g2:
            if freeze_base:
                children.tau_e = p1.tau_e.clone()
                children.w_ei = p1.w_ei.clone()
                children.w_ie = p1.w_ie.clone()
            else:
        ...
        # ---- G3（期相激素 v1）：按列交叉 + 高斯变异（无掩码；FREEZE_BASE 不冻结激素）----
        col3 = torch.rand(B2, N, device=dev) > 0.5
        children.W_rel_e = torch.where(col3.unsqueeze(2), p1.W_rel_e, p2.W_rel_e)
        children.W_rel_i = torch.where(col3.unsqueeze(2), p1.W_rel_i, p2.W_rel_i)
        children.b_rel_e = torch.where(col3, p1.b_rel_e, p2.b_rel_e)
        children.b_rel_i = torch.where(col3, p1.b_rel_i, p2.b_rel_i)
        for attr in GeneStack.G3_HORMONE:
            t = getattr(children, attr)
            v = lambda x: x.view(B2, *([1] * (t.dim() - 1))).to(t.dtype)
            noise = torch.randn_like(t) * v(horm_std1)
            m = (torch.rand_like(t) < v(horm_frac1)).to(t.dtype)
            setattr(children, attr, t + noise * m)
...
def load_best_state(path, cfg):
    ...
    if st is None:
        return None
    # 迁移通道：旧格式（test12，无 BRAIN_VERSION）允许读取——
    # set_individual_from_state 对缺失激素键保持随机初始化（血统迁移路径）。
    # 仅当携带不同版本号时拒绝。
    saved_bv = saved_cfg.get('BRAIN_VERSION')
    if saved_bv is not None and saved_bv != getattr(cfg, 'BRAIN_VERSION', 'horm1'):
        print(f"  警告: 模型 {path} 基因组版本不符 "
              f"({saved_bv} != {cfg.BRAIN_VERSION})，已忽略种子")
        return None
...
def send_autodl_notify(cfg, title, content):
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            data=json.dumps({'title': title[:20], 'name': 'test14 期相激素进化',
...
def load_checkpoint7(path, cfg):
    ...
    if saved_cfg and saved_cfg.get('OBS_ENC_VERSION', '32proj') != getattr(cfg, 'OBS_ENC_VERSION', '32proj'):
        ...
        return None
    # 基因组版本守卫：缺 BRAIN_VERSION 键（test12 旧断点）或版本不符 → 拒绝续训
    # （不可给 get 设默认值——缺键必须视为旧格式。单脑迁移走 load_best_state，
    # 那里允许旧格式以支持 --migrate-from。）
    if saved_cfg and saved_cfg.get('BRAIN_VERSION') != getattr(cfg, 'BRAIN_VERSION', 'horm1'):
...
def run_training(cfg):
    ...
    print(f"[激素v1] enable={cfg.HORMONE_ENABLE} fin={cfg.HORMONE_FIN} "
          f"λ={cfg.HORMONE_DECAY} dif={cfg.EXCIT_DIFFUSION}/{cfg.INHIB_DIFFUSION} "
          f"tau增益={cfg.HORMONE_TAU_E_GAIN}/{cfg.HORMONE_TAU_I_GAIN} "
          f"增益={'开' + str(cfg.HORMONE_GAIN_SCALE) if cfg.HORMONE_GAIN_ENABLE else '关'} "
          f"每{cfg.HORMONE_APPLY_EVERY}步 | freeze_base={cfg.FREEZE_BASE} "
          f"init_std={cfg.HORMONE_INIT_STD} mut={cfg.HORMONE_MUT_STD}/{cfg.HORMONE_MUT_FRAC}")
    ...
    if cfg.AUTO_RESUME:
        ...
        if ck is not None:
            ...
            for k in ('elite_food', 'best_fit', 'best_turneff',
                      ...
                      'fit_parts_food', 'fit_parts_eff',
                      'fit_parts_straight', 'fit_parts_edge',
                      'fit_parts_noconn', 'horm_e', 'horm_i', 'horm_food_corr'):
    ...
    try:
        for gen in range(start_gen, cfg.GENERATIONS):
            ...
            # 激素遥测：种群平均场强 + 场强-食物跨个体相关（>0 ⇒ 激素活动与表现相关）
            if mn.shape[1] > 20:
                history['horm_e'].append(float(np.mean(mn[:, 19])))
                history['horm_i'].append(float(np.mean(mn[:, 20])))
                if float(np.std(mn[:, 19])) > 1e-9:
                    hc = float(np.corrcoef(mn[:, 19], mn[:, 0])[0, 1])
                else:
                    hc = 0.0
                history['horm_food_corr'].append(0.0 if np.isnan(hc) else hc)
            else:
                history['horm_e'].append(0.0)
                history['horm_i'].append(0.0)
                history['horm_food_corr'].append(0.0)
...
def make_smoke_config():
    ...
    cfg.CHECKPOINT_PATH = 'test14_smoke_checkpoint.pth'
    cfg.BEST_MODEL_PATH = 'test14_smoke_best.pth'
    cfg.LATEST_GEN_BEST_MODEL_PATH = 'test14_smoke_latest_gen_best.pth'
    cfg.HISTORY_JSON_PATH = 'test14_smoke_history.json'
...
def main():
    ap = argparse.ArgumentParser(description='test14 — 期相激素 v1（CRN + 两阶段淘汰 + 类正态变异 + G3 激素组）')
    ...
    ap.add_argument('--arm', type=str, default=None, choices=['C0', 'H1', 'H2'],
                    help='mini A/B 臂：C0=零激素对照 | H1=冻结底盘只进化激素 | H2=全协同')
    ap.add_argument('--no-hormone', dest='no_hormone', action='store_true',
                    help='评估期关闭激素（与 test12 严格同数学；C0 臂/消融用）')
    ap.add_argument('--no-gain', dest='no_gain', action='store_true',
                    help='关闭输入群组增益调制目标（仅保留 tau 调制）')
    ap.add_argument('--migrate-from', dest='migrate_from', type=str, default=None,
                    help='test12 冠军单脑迁移：全种群注入该底盘，激素基因保持随机初始化')
    ap.add_argument('--horm-std', type=float, default=None,
                    help='激素基因初始化/参考尺度 HORMONE_INIT_STD（默认 0.02）')
    ...
    if args.fit_mode:
        ...
        if not args.smoke:
            ...
            cfg.CHECKPOINT_PATH = f'test14_{arm}_checkpoint.pth'
            cfg.BEST_MODEL_PATH = f'test14_{arm}_best_model.pth'
            cfg.LATEST_GEN_BEST_MODEL_PATH = f'test14_{arm}_latest_gen_best.pth'
            cfg.HISTORY_JSON_PATH = f'test14_{arm}_history.json'
    ...
    if args.name:
        cfg.CHECKPOINT_PATH = f'test14_{args.name}_checkpoint.pth'
        cfg.BEST_MODEL_PATH = f'test14_{args.name}_best_model.pth'
        cfg.LATEST_GEN_BEST_MODEL_PATH = f'test14_{args.name}_latest_gen_best.pth'
        cfg.HISTORY_JSON_PATH = f'test14_{args.name}_history.json'
    if args.arm:
        if args.arm == 'C0':
            cfg.HORMONE_ENABLE = False      # 零激素对照：与 test12 严格同数学
        elif args.arm == 'H1':
            cfg.FREEZE_BASE = True          # 冻结底盘，只进化激素基因
        if not args.name:
            nm = args.arm.lower()
            cfg.CHECKPOINT_PATH = f'test14_{nm}_checkpoint.pth'
            cfg.BEST_MODEL_PATH = f'test14_{nm}_best_model.pth'
            cfg.LATEST_GEN_BEST_MODEL_PATH = f'test14_{nm}_latest_gen_best.pth'
            cfg.HISTORY_JSON_PATH = f'test14_{nm}_history.json'
    if args.no_hormone:
        cfg.HORMONE_ENABLE = False
    if args.no_gain:
        cfg.HORMONE_GAIN_ENABLE = False
    if args.horm_std is not None:
        cfg.HORMONE_INIT_STD = args.horm_std
    if args.migrate_from:
        cfg.SEED_FROM_BEST = True
        cfg.SEED_MODEL_PATH = args.migrate_from
        cfg.SEED_POP = True
    if args.resume_pop:
    ...
    try:
        ...
        send_autodl_notify(cfg, 'test14 训练异常退出',
