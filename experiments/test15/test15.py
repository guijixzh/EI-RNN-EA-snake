# ==========================================
# test15.py —— 观测增维 32→40（期相信息直供；test12 血统迁移，一期一杠杆）
#
# 动机（证据链）：Phase-0 归因（results/phase_diagnostic.json）确认后期游戏段
#   模式缺失（冠军脑置身 len≥30-40 身体即崩塌，主死因空间挤压自撞，对教师
#   差距 ~60 分）；test13 判定"32 维局部扇区观测不携带全局占用/规划所需信息
#   ——表示天花板"；test14 证明纯动态调制（期相激素 v1）无可测量救援
#   （G1-NULL）。本轮把缺失的信息直接补进观测。
#
# 新 8 通道（追加 [32:40]，全部 ego 相对系，×OBS_NEW_SCALE=8）：
#   [32]    饥饿钟压力 steps_wo_food/(SLOPE·len+20)——追/规换挡紧急度。
#   [33:37] 尾相对方位 前/右/后/左 4 通道：sig=clamp((tail−head)·û_i,0)/曼哈顿
#           距离，与食物块 [8:16] 同族同序（补绝对系尾 one-hot [4:8] 的方向
#           缺失；方向相对性审计见 docs/test15_experiment_log.md）。
#   [37:40] 前/左/右 7 步有限洪水稀缺度：种子=该方向邻格（堵死=稀缺度 1）、
#           蛇身严格占用（尾挡路，FLOOD_TAIL_BLOCK 可关）、深度≤7 BFS 可达
#           自由格数 c；稀缺度=clamp(1−c/空盘同位置可达数,0,1)（空盘可达数
#           逐格 BFS 预计算 [100] 查表，位置归一化）。与动作空间{直,左,右}
#           近乎同构=每动作局部价值估计；实现=max_pool 交叉扩散 7 迭代。
#
# A/B 设计：OBS_NEW_ENABLED=False 即新通道置零（A0 对照，与 test12 严格同
#   数学、同 CRN 配对）；--migrate-from 做 M_in/W_in 列扩展迁移（旧 32 列
#   复制 + 新 8 列 INIT_DENSITY 激活/σ=0.1）。判定门与结果：
#   docs/test15_experiment_log.md、experiments/exp_obs40_calibrate.py。
# ==========================================
...
class Config:
    ...
    NUM_COLUMNS = 256
    OBS_MODE = '32proj'
    OBS_DIM = 24 if OBS_MODE == '24' else 40    # test15：32 旧通道 + 8 新通道
    ACTION_DIM = 3
    INIT_DENSITY = 0.15
    ...
    BRAIN_VERSION = 'base1'     # 基因组版本守卫（OBS_DIM=40 基因组）
...
def load_migratable_state(path, cfg):
    """test12 旧单脑（OBS_DIM=32）→ test15 40 维迁移变换。

    M_in/W_in 旧 32 列原样复制（保迁移行为基线），新 8 列按 INIT_DENSITY
    激活 / σ=0.1（test7a 证据：强信号×随机权重=立即可被选择的变异，非死
    通道情形）。固定种子生成器保证迁移可复现。"""
    if not path or not os.path.exists(path):
        return None
    try:
        ...
        print(f"  警告: 迁移源 {path} 读取失败 ({e})")
        return None
    saved = data.get('config', {})
    st = data.get('brain')
    if st is None:
        return None
    if int(saved.get('NUM_COLUMNS', -1)) != cfg.NUM_COLUMNS or \
            int(saved.get('ACTION_DIM', -1)) != cfg.ACTION_DIM:
        print(f"  警告: 迁移源 {path} N/ACTION_DIM 不符，放弃迁移")
        return None
    if int(saved.get('OBS_DIM', 32)) != 32:
        print(f"  警告: 迁移源 {path} 须为旧 32 维编码（实测 {saved.get('OBS_DIM')}）")
        return None
    st = dict(st)
    N = cfg.NUM_COLUMNS
    O_old, O_new = 32, cfg.OBS_DIM
    gen = torch.Generator().manual_seed(20260902)
    m_new = (torch.rand(N, O_new - O_old, generator=gen) < cfg.INIT_DENSITY).float()
    w_new = torch.randn(N, O_new - O_old, generator=gen) * 0.1
    st['M_in'] = torch.cat([st['M_in'].float(), m_new], dim=1)
    st['W_in'] = torch.cat([st['W_in'].float(), w_new], dim=1)
    return st, float(data.get('food', -1.0)), float(data.get('steps', 0.0))
...
def send_autodl_notify(cfg, title, content):
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            data=json.dumps({'title': title[:20], 'name': 'test15 观测增维进化',
...
def load_checkpoint7(path, cfg):
    ...
    # （旧 32 维血统走 --migrate-from 迁移通道，不经此处）
    if saved_cfg and saved_cfg.get('BRAIN_VERSION') != getattr(cfg, 'BRAIN_VERSION', 'base1'):
...
def make_smoke_config():
    ...
    cfg.CHECKPOINT_PATH = 'test15_smoke_checkpoint.pth'
    cfg.BEST_MODEL_PATH = 'test15_smoke_best.pth'
    cfg.LATEST_GEN_BEST_MODEL_PATH = 'test15_smoke_latest_gen_best.pth'
    cfg.HISTORY_JSON_PATH = 'test15_smoke_history.json'
...
def main():
    ap = argparse.ArgumentParser(description='test15 — 观测增维 32→40（CRN + 两阶段淘汰 + 类正态变异）')
    ...
    if args.fit_mode:
        ...
        if not args.smoke:
            ...
            cfg.CHECKPOINT_PATH = f'test15_{arm}_checkpoint.pth'
            cfg.BEST_MODEL_PATH = f'test15_{arm}_best_model.pth'
            cfg.LATEST_GEN_BEST_MODEL_PATH = f'test15_{arm}_latest_gen_best.pth'
            cfg.HISTORY_JSON_PATH = f'test15_{arm}_history.json'
    ...
    if args.name:
        cfg.CHECKPOINT_PATH = f'test15_{args.name}_checkpoint.pth'
        cfg.BEST_MODEL_PATH = f'test15_{args.name}_best_model.pth'
        cfg.LATEST_GEN_BEST_MODEL_PATH = f'test15_{args.name}_latest_gen_best.pth'
        cfg.HISTORY_JSON_PATH = f'test15_{args.name}_history.json'
    if args.resume_pop:
    ...
    try:
        ...
        send_autodl_notify(cfg, 'test15 训练异常退出',
