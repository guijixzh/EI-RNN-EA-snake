# ==========================================
# test16.py —— 循环权重固定扇入稀疏化（N=1024；test15 血统，环境/参数逐字继承）
#
# 动机：把 NUM_COLUMNS 256→1024 在稠密 N×N 表示下意味着 W_rec/M_rec/W_rec_eff
#   三个 [B,N,N] 张量 ×16（每体 6.3MB、种群栈 12.9GB、checkpoint 6.4GB、
#   每步权重流式 2.1MB/拷贝）。而结构本身极稀疏，本轮规定**每个突触后神经元
#   恰有 REC_FANIN=16 个输入槽**，用槽位布局取代稠密矩阵：
#
# 表示（rec 三张量 → 两个）：
#   rec_idx [B,N,K] int64  —— 每行 K 个源神经元 id（自连禁止；重复源=权重叠加，
#                            等效"≤16 连接"约束；落盘转 int16 省盘）
#   rec_w   [B,N,K] half   —— 槽位权重；M_rec 概念消失（索引即掩码），
#                            W_rec_eff 不再需要（权重天生稀疏）
# 前向：rec = gather(E, dim=1, rec_idx)·rec_w 逐槽求和 —— MACs 1M→16K（×64）、
#   每步权重流量 2.1MB→~0.16MB（×13），种群栈 →~0.4GB，checkpoint →~0.4GB。
#
# 与 test15 的语义差异（三处，均已在此注明）：
#   1. rec 交叉由稠密"行×列双掩码+冲突掷币"改为**按行选父**（每突触后神经元
#      整行继承 p1 或 p2，保持扇入结构）。
#   2. 拓扑变异的"M_rec"分支变为**槽位重连**（随机新目标 j≠自身）。
#   3. 修复 test12 血统休眠代隐患：CYCLE_PATTERN 改 ('G2','G1')（2 代轮换），
#      且冻结组基因改为**单亲（p1）遗传**——旧实现冻结组子代张量保持
#      torch.empty 未初始化，靠 CUDA 缓存分配器返回陈旧内存才未爆雷。
#
# 不变项：obs40 全部编码（含洪水稀缺度）、BatchedSnakeEnv、CRN、两阶段淘汰、
#   19 列指标、econ v7 适应度、STARVE_SLOPE=5、K=5 帧思考、CLI 骨架。
# 版本守卫：BRAIN_VERSION='sparse1'（test15 及更早稠密断点自动拒绝；
#   稠密→稀疏无迁移通道，从零训练）。环境与参数基于 test15（OBS_NEW_ENABLED=True）。
# ==========================================
...
class Config:
    ...
    W_STRAIGHT = 0.5            # 少转弯因子（straight=1−转弯/步）
    ...
    # --- 断点 / 最优模型 / 种子 ---
    CHECKPOINT_PATH = 'test16_checkpoint.pth'
    BEST_MODEL_PATH = 'test16_best_model.pth'
    LATEST_GEN_BEST_MODEL_PATH = 'test16_latest_gen_best.pth'
    HISTORY_JSON_PATH = 'test16_history.json'
    AUTO_RESUME = True
...
def send_autodl_notify(cfg, title, content):
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            data=json.dumps({'title': title[:20], 'name': 'test16 稀疏固定扇入进化',
...
def make_smoke_config():
    ...
    cfg.CHECKPOINT_PATH = 'test16_smoke_checkpoint.pth'
    cfg.BEST_MODEL_PATH = 'test16_smoke_best.pth'
    cfg.LATEST_GEN_BEST_MODEL_PATH = 'test16_smoke_latest_gen_best.pth'
    cfg.HISTORY_JSON_PATH = 'test16_smoke_history.json'
...
def main():
    ap = argparse.ArgumentParser(
        description='test16 — 循环权重固定扇入稀疏化 N=1024（CRN + 两阶段淘汰 + 类正态变异）')
    ...
    if args.fit_mode:
        ...
        if not args.smoke:
            ...
            cfg.CHECKPOINT_PATH = f'test16_{arm}_checkpoint.pth'
            cfg.BEST_MODEL_PATH = f'test16_{arm}_best_model.pth'
            cfg.LATEST_GEN_BEST_MODEL_PATH = f'test16_{arm}_latest_gen_best.pth'
            cfg.HISTORY_JSON_PATH = f'test16_{arm}_history.json'
    ...
    if args.name:
        cfg.CHECKPOINT_PATH = f'test16_{args.name}_checkpoint.pth'
        cfg.BEST_MODEL_PATH = f'test16_{args.name}_best_model.pth'
        cfg.LATEST_GEN_BEST_MODEL_PATH = f'test16_{args.name}_latest_gen_best.pth'
        cfg.HISTORY_JSON_PATH = f'test16_{args.name}_history.json'
    if args.resume_pop:
    ...
    try:
        ...
        send_autodl_notify(cfg, 'test16 训练异常退出',
