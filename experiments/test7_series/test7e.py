# ==========================================
# test7e.py —— 转弯密度计价版（基于 test7d，诊断驱动）
#
# 诊断结论（test7d_diag.py，test7b 67 分模型 200 局）：
#  - 摆动是强烈决定的政策惯用手法（转向决策 top-2 边际均值 6.9，仅 4.3% <0.3），
#    非 argmax 抖动 → 滞回解码默认关闭（TURN_MARGIN=0，仅留开关）；
#  - 无疲劳军备竞赛（历代 W_out 范数无膨胀）；
#  - 转向密度随蛇长上升（0.39→0.51），直行游程 69%≤2 格；
#    高密度窗口自由格连通域占比显著更低（0.696 vs 0.760）→ 密集转弯碎片化空间。
#
# 相对 test7d 的改动：
#  1. econ 成本的转向项改为整局转向数（act1+act2，含死亡前挣扎）：
#     cost = steps_last + TURN_COST·(1+CROWD_W·food/50)·turns_total
#     挣扎摆动与长蛇期摆动同样计价；steps 仍只计到最后一食（不奖励快死）。
#  2. TURN_MARGIN 滞回解码开关（默认 0=关）。
#  3. 路径 test7e_*，种子默认 test7d_econ_best_model.pth。
# 转向疲劳（gain=0.2/decay=0.9）、饿死斜率（3*len+20）维持。
# ==========================================
...
class Config:
    ...
    # --- 断点 / 最优模型 / 种子 ---
    CHECKPOINT_PATH = 'test7e_econ_checkpoint.pth'
    BEST_MODEL_PATH = 'test7e_econ_best_model.pth'
    LATEST_GEN_BEST_MODEL_PATH = 'test7e_econ_latest_gen_best.pth'
    AUTO_RESUME = True
    CHECKPOINT_INTERVAL = 10
    SEED_FROM_BEST = False
    SEED_MODEL_PATH = 'test7d_econ_best_model.pth'
    SEED_MODEL_PATH2 = 'test7d_econ_latest_gen_best.pth'
...
def make_smoke_config():
    ...
    cfg.CHECKPOINT_PATH = 'test7e_smoke_checkpoint.pth'
    cfg.BEST_MODEL_PATH = 'test7e_smoke_best.pth'
    cfg.LATEST_GEN_BEST_MODEL_PATH = 'test7e_smoke_latest_gen_best.pth'
...
def main():
    ...
    ap.add_argument('--fit-mode', type=str, default=None, choices=['econ', 'additive', 'tuple'],
                    help='适应度模式（默认 econ）；决定文件前缀 test7e_econ_*/test7e_add_*')
    ...
    if args.fit_mode:
        ...
        if not args.smoke:
            ...
            cfg.CHECKPOINT_PATH = f'test7e_{arm}_checkpoint.pth'
            cfg.BEST_MODEL_PATH = f'test7e_{arm}_best_model.pth'
            cfg.LATEST_GEN_BEST_MODEL_PATH = f'test7e_{arm}_latest_gen_best.pth'
