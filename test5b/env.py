...
# 额外新增 _get_grid_state() -> 10x10x5 多通道网格，供 CNN 前端使用。
#
# 动作：0=直行, 1=左转, 2=右转
# ==========================================
class SnakeEnv:
    def __init__(self, grid_size=10):
        ...
        self.obs_dim = 24  # 与 Config.OBS_DIM 一致（test5a 兼容）
        self.reset()
    ...
    def _place_food(self):
        ...
        修复：原实现「随机抽格子撞空格」在蛇身占满棋盘时命中概率趋近 0，
        期望重试次数 = 100/空格数 爆炸式增长，甚至无限死循环。

        返回 True=放置成功；False=棋盘已满（无可放位置，游戏胜利结束）。
        """
        gs = self.grid_size
