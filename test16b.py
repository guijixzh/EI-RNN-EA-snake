# ==========================================
# test16b.py —— 评估提速（fast-eval）+ 阶段2 对半精评（halving）；test16a 子版本
#
# 动机：云端 simp 臂（pop4096, K1=12→保2048→K2=24, 精英1024=种群1/4）单代
#   eval ~1300s，剩余 110 代 ≈ 40 GPU·h。诊断（docs/test16b_experiment_log.md）：
#   每环境步 ~1200-1500 次 Python 级 torch 调用 + 每步 3-4 次强制同步，GPU 利用率
#   ~0.03%——瓶颈是每步调度开销而非局数/FLOPs；且 _fitness_simple 只吃 4 列
#   （food/steps_last/act1/act2），其余 15 列遥测占热路径 ~450 op/步。
#
# A 层 --fast-eval（默认开，--no-fast-eval 关；选择零影响——适应度路径逐位不变）：
#   1. obs40 16 扇区射线向量化：预计算 [8,G,2] 偏移 + 一次 gather + cumsum 首命中，
#      ~960 op/步 → ~30 op（首命中/出界语义与逐 k 循环逐位一致，自检 8 对拍）。
#   2. 删除 test16a.py:940-951 食物方位块的重复执行（两遍逐字相同，删第二遍）。
#   3. 遥测降频不减列（列 5/11/12/13/14/15/16/18 全保留，采样口径变化）：
#      teacher 每 8 步、edge 每 4 步（新增 edge_steps 归一化分母，straight 分母
#      beh_steps 仍为全存活步）、conn 每 32 步、reach 每 16 步（窗口近似：吃食后
#      ≤16 步内测 reach）；al.any()/ate_now.any() 每步门控同步消除（掩码无条件累计）。
#   4. all_done 每 8 步查一次（死个体冻结，最多多跑 7 步全死批）。
#   5. EVAL_MEM_FRAC 0.55→0.85（更大分块、更少扫描；launch-bound 下墙钟∝扫描次数）
#      + 分块 OOM 自动减半重试。
#   6. 阶段2 分块按阶段1 实测 steps_last 降序组装（长命个体聚一块，短命块提前
#      收工；逐行结果与分组无关——纯调度）。
#
# B 层 --stage2-halving（默认开，--no-stage2-halving 关；口径变化，云端直改）：
#   阶段2 幸存者全体先打 A=6 局 → 按 key(K1+A) 排序取前 ELITE_SIZE 再打 B=18 局。
#   精英最终 K = K1+24=36 与 test16a 完全一致（一食不动），注定落选的后一半每体
#   省 18 局。云端档阶段2 49,152→30,720 局（-37.5%）。--stage2a-eps/--stage2b-eps 可调。
#   备选 --k2-floor-dynamic（默认关）：halving 关闭时 K2 地板改用
#   need=⌈(SEL_CV·S/δ)²⌉−K1（σ_ε≤δ 预注册判据）。
#
# 其他：--profile-eval 分块计时探针（obs/fwd/teacher/edge/conn/reach/step 占比，
#   同步计时会拖慢绝对速度，仅看占比）；--seed 固定 CPU+CUDA 随机种子（A/B 对照）。
# 基因组/CRN/适应度公式与 test16a 逐字一致（BRAIN_VERSION='sparse1'、
#   FITNESS_VERSION 9 不变），--resume-pop test16a_simp_checkpoint.pth 可直接从
#   gen210 断点续训。
# ==========================================
...
class Config:
    ...
    # --- 断点 / 最优模型 / 种子 ---
    CHECKPOINT_PATH = 'test16b_checkpoint.pth'
    BEST_MODEL_PATH = 'test16b_best_model.pth'
    LATEST_GEN_BEST_MODEL_PATH = 'test16b_latest_gen_best.pth'
    HISTORY_JSON_PATH = 'test16b_history.json'
    AUTO_RESUME = True
...
def evaluate_population_halving(pop, cfg, gen=0):
    ...
    m2b = _eval_pop_banks(sub_top, cfg,
    ...
    try:
        ...
        req = urllib.request.Request(
            'https://www.autodl.com/api/v1/wechat/message/send',
                data=json.dumps({'title': title[:20], 'name': 'test16b 提速评估进化',
...
def run_training(cfg):
    ...
    if getattr(cfg, 'STAGE2_HALVING', False) and cfg.STAGE2_KEEP > cfg.ELITE_SIZE:
        print(f"[test16b halving] 阶段2 对半精评：全体幸存者 ×{cfg.STAGE2A_EPS} 局 → "
...
def make_smoke_config():
    ...
    cfg.CHECKPOINT_PATH = 'test16b_smoke_checkpoint.pth'
    cfg.BEST_MODEL_PATH = 'test16b_smoke_best.pth'
    cfg.LATEST_GEN_BEST_MODEL_PATH = 'test16b_smoke_latest_gen_best.pth'
    cfg.HISTORY_JSON_PATH = 'test16b_smoke_history.json'
...
def main():
    ap = argparse.ArgumentParser(
        description='test16b — fast-eval 评估提速 + 阶段2 对半精评（兼容 test16a/test16 种群）')
    ...
    if args.fit_mode:
        ...
        if not args.smoke:
            ...
            cfg.CHECKPOINT_PATH = f'test16b_{arm}_checkpoint.pth'
            cfg.BEST_MODEL_PATH = f'test16b_{arm}_best_model.pth'
            cfg.LATEST_GEN_BEST_MODEL_PATH = f'test16b_{arm}_latest_gen_best.pth'
            cfg.HISTORY_JSON_PATH = f'test16b_{arm}_history.json'
    ...
    if args.name:
        cfg.CHECKPOINT_PATH = f'test16b_{args.name}_checkpoint.pth'
        cfg.BEST_MODEL_PATH = f'test16b_{args.name}_best_model.pth'
        cfg.LATEST_GEN_BEST_MODEL_PATH = f'test16b_{args.name}_latest_gen_best.pth'
        cfg.HISTORY_JSON_PATH = f'test16b_{args.name}_history.json'
    if args.resume_pop:
    ...
    try:
        ...
        send_autodl_notify(cfg, 'test16b 训练异常退出',
