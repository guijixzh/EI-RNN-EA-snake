...
class GeneStack:
    ...
    def all_genes(self):
        """实际持有的基因名集合（激素基因仅在已分配时纳入保存/加载/索引）。"""
        if getattr(self, 'Wh1', None) is not None:
            return self.GENES + self.HORMONE_GENES
        return self.GENES
...
def deliberate_batch(pop, obs, E, I, st, press, cfg, he=None, hi=None):
    ...
    for k in range(K):
        ...
        logits, E, I, st, he, hi = forward_batch(pop, o, E, I, st, press, cfg, he, hi)
...
def selfcheck(cfg):
    ...
    verdict = ok9 and ok10 and ok11 and ok12 and ok13 and ok14
    print(f"=== 扩展自检（9-14）{'全部通过' if verdict else '存在 FAIL'} ===")
    print("=== 自检完成 ===")
